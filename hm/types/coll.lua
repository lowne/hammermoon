...
---Some (vaguely FP) constructs for collections in Lua tables.
...
-- Notes:
--
--   - you can also use the shortcut `coll(my_table)`
--
...
-- @returns #coll.set the resulting set
...
-- @param #coll self
-- @returns #coll.map the resulting map
...
function f.ireduce(t,fn,...)
  ...
  for k=i,#t do tinsert(r,t[k]) tinsert(r,k) r=tpack(fn(tunpack(r))) end
  return tunpack(r)
end
...
function f.reduce(t,fn,...)
  ...
  k,v=next(t,k)
  while k~=nil do tinsert(r,v) tinsert(r,k) r=tpack(fn(tunpack(r))) k,v=next(t,k) end
  return tunpack(r)
end
